...
import org.apache.spark.sql.catalyst.expressions.{And, Attribute, AttributeMap, AttributeReference, EqualTo, GreaterThan, GreaterThanOrEqual, IsNotNull, LessThan, LessThanOrEqual, Literal, Not}
...
class FilterJoinEstimationSuite extends StatsEstimationTestBase {
   ...
   */
  test(
    """3 tables disjoint left outer join and inner join
    where true""") {
 ...
 */
  test(
    """3 tables left outer join and inner join with multi condiction
    where true""") {
 ...
 */
  test(
    """3 tables left outer join and inner join with multi condiction2
    where Not(t1.cint_1_200 < 3 AND t2.cfloat_200 >= 150.0)""") {
...
*
* select *
* from table1 t1 left semi join table4 t4 on t1.cdouble_200 = t4.cdouble_50_149
* left anti join table2 t2 on t1.cdecimal_200 = t2.cdecimal_100
* where true
*
*/
  test(
    """3 tables left semi join and left anti join with  condiction
    where true""") {

    /** execute join */
    val joinType = Some(EqualTo(nameToAttr("cint_1_200"), nameToAttr("cint_1_100")))

    val join = Join(table1, table4, LeftSemi, joinType)

    val joinType2 = Some(EqualTo(nameToAttr("cdecimal_200"), nameToAttr("cdecimal_100")))

    val join2 = Join(join, table2, LeftAnti, joinType2)

    /** join size define */
    val joinSizeInBytes = BigInt(200L * (8 + (4 + 8 + 8 + 4)))
    val joinRowCount = BigInt(200L)
...
* from table1 t1 left semi join table4 t4 on t1.cdouble_200 = t4.cdouble_50_149
* left anti join table2 t2 on t1.cdecimal_200 = t2.cdecimal_100
* where t1.cdate_200 >= Date('2019-01-31') and t1.cint_1_200 <= 100
* and t1.cdouble_200 <= 80.0
*
*/
  test(
    """3 tables left semi join and left anti join with  condiction
    where t1.cdate_200 >= Date('2019-01-31') and t1.cint_1_200 <= 100 and
     t1.cdouble_200 <= 80.0""") {
    ...
    /** define filter condition */
    val d20190131 = DateTimeUtils.fromJavaDate(Date.valueOf("2019-01-31"))

    val subCondition = And(GreaterThanOrEqual(attrDate_200, Literal(d20190131, DateType)),
      LessThanOrEqual(attrInt_1_200, Literal(100)))

    val condition = And(subCondition, LessThanOrEqual(attrDouble_200,
      Literal(80.0, DoubleType)))

    /** define expected stats */
    val expectedRowCount = 34L
    val expectedColStats = Seq(
      nameToAttr("cint_1_200") -> ColumnStat(distinctCount = 34,
        min = Some(1), max = Some(100), nullCount = 0, avgLen = 4, maxLen = 4),

      nameToAttr("cdouble_200") -> ColumnStat(distinctCount = 34,
        min = Some(1.0), max = Some(80.0), nullCount = 0, avgLen = 8, maxLen = 8),

      nameToAttr("cdecimal_200") -> ColumnStat(distinctCount = 34,
        min = Some(decMin_200), max = Some(decMax_200), nullCount = 0, avgLen = 8, maxLen = 8),

      nameToAttr("cdate_200") -> ColumnStat(distinctCount = 34,
        min = Some(DateTimeUtils.fromJavaDate(Date.valueOf("2019-01-31"))),
        max = Some(dMax_200), nullCount = 0, avgLen = 4, maxLen = 4))
